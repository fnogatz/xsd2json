...
function complexTypesToMerge(complexTypes) {
	...
	_.each(complexTypes, function(complexType) {
		...
		if (extendedType) {
			...
			return log.error('Complex type %s should extend other type %s but it is not defined', complexType.$.name, extendedTypeKey);
		}
	});
...
function mergeComplexTypes(complexTypeInheritance) {
	...
	_.deepSet(child, 'xs:sequence.0.xs:group', _.union(_.deepGet(parent, 'xs:sequence.0.xs:group'), _.deepGet(child, 'xs:complexContent.0.xs:extension.0.xs:sequence.0.xs:group')));
	...
	_.deepSet(child, 'xs:all.0.xs:group', _.union(_.deepGet(parent, 'xs:all.0.xs:group'), _.deepGet(child, 'xs:complexContent.0.xs:extension.0.xs:all.0.xs:group')));
...
function fixComplexType(complexType, groupsMap) {
	...
	//if (complexType.$.name.indexOf('OrderType') !== -1) console.log(JSON.stringify(complexType, null, 2));
...
exports.mergeExtensions = function(schemaParam) {
	...
	var complexTypes = schema['xs:schema']['xs:complexType'] = splitInheritedChoices(schema['xs:schema']['xs:complexType']);
